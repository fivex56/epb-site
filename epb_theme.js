...
(function (window, document) {
  ...
  /* Пустая ячейка отвечает на два разных вопроса, и путать их нельзя:
     «этого тут не продают» — факт о площадке, «мы не прочитали» — факт о нас.
     notSold() истинно только для первого, и только там, где есть источник. */
  EPB.notSold = function (p, key) {
    return ((p && p.not_sold_keys) || []).indexOf(key) !== -1;
  };
  EPB.notSoldTitle = function (p, key) {
    if (!EPB.notSold(p, key)) return '';
    var name = (p.platform_name || p.platform_id || 'This platform');
    var src = p.terms_source ? ' ' + p.terms_source : '';
    return name + ' does not sell this rental term, so there is no price to show.' + src;
  };
  /* Третье состояние пустой ячейки: мы не установили, продаёт ли площадка этот
     срок вообще. Ни цены, ни обвинения в её адрес — но и не молчание: молчащая
     пустота читается читателем как «не умеют». */
  EPB.termUnknown = function (p, key) {
    return ((p && p.unknown_keys) || []).indexOf(key) !== -1;
  };
  EPB.termUnknownTitle = function (p, key) {
    if (!EPB.termUnknown(p, key)) return '';
    var name = (p.platform_name || p.platform_id || 'This platform');
    return 'We have not established whether ' + name + ' sells this rental term, so the ' +
           'board neither prices it nor calls it unavailable.';
  };
  /* A volume the platform will not sell. The number in the cell is exact - it is
     the same per-unit rate, priced pro rata - but the order cannot be placed, so
     it is marked apart from an estimate rather than lumped in with one. */
  EPB.orderBound = function (p, key) {
    if (!p || !key) return '';
    if (((p.below_min_keys) || []).indexOf(key) !== -1) return 'min';
    if (((p.above_max_keys) || []).indexOf(key) !== -1) return 'max';
    if (((p.off_step_keys) || []).indexOf(key) !== -1) return 'step';
    return '';
  };
  EPB.stepList = function (p) {
    return ((p && p.order_steps) || []).map(EPB.energyShort).join(' / ');
  };
  EPB.energyShort = function (n) {
    n = +n;
    if (!isFinite(n) || n <= 0) return '';
    return n >= 1e6 ? (n / 1e6).toFixed(n % 1e6 ? 1 : 0) + 'M' : Math.round(n / 1e3) + 'k';
  };
  /* 'min 100k' - short enough to sit inside a price cell. */
  EPB.boundLabel = function (p, key) {
    var b = EPB.orderBound(p, key);
    if (!b) return '';
    if (b === 'step') return 'fixed sizes';
    var n = EPB.energyShort(b === 'min' ? p.minimum_order_energy : p.maximum_order_energy);
    return n ? b + ' ' + n : b + ' order size';
  };
  EPB.boundTitle = function (p, key) {
    var b = EPB.orderBound(p, key);
    if (!b) return '';
    var name = (p.platform_name || p.platform_id || 'This platform');
    var tail;
    if (b === 'step') {
      /* We read a list of options, which is not the same evidence as a rule
         forbidding everything else. The sentence says what we read. */
      tail = ' sells fixed amounts: ' + EPB.stepList(p) + '. This volume is not one of them.';
    } else {
      var n = (+(b === 'min' ? p.minimum_order_energy : p.maximum_order_energy)).toLocaleString('en-US');
      tail = (b === 'min'
        ? ' will not sell an order this small: its minimum is ' + n + ' energy.'
        : ' will not sell an order this large: its maximum is ' + n + ' energy.');
    }
    return name + tail +
      ' The price is exact - the same per-unit rate priced pro rata, so the row stays' +
      ' comparable with the rest - but you could not place this order at this volume.';
  };
  ...
  EPB.noteMark = function (p, label) {
    ...
    if (!n) return '';
    var t = EPB.esc(n);
    /* A button, so Tab reaches it: this sentence is often the reason a
       platform's row looks odd, and it was mouse-only. The label carries the
       whole note, so the separate sr-only copy went - it was being read
       twice. The popup exists because a native title never opens for a
       keyboard, and the stylesheet has had a :focus rule for it all along. */
    var lbl = EPB.esc('How ' + (label || 'this') + ' price is read: ') + t;
    return '<span class="note-wrap">' +
      '<button type="button" class="note-mark" title="' + t + '" aria-label="' + lbl + '"></button>' +
      '<span class="note-pop" aria-hidden="true">' + t + '</span></span>';
  };
  ...
  /* Одна линия на платформу вместо десяти. Каждый тариф приводится к общей
     мерке — TRX за 65 000 энергии за час, — и в каждый момент времени берётся
     среднее по тем тарифам, которые у платформы есть. Тарифы равноправны:
     чем взвешивать, мы не знаем, а выдуманный вес хуже отсутствующего.

     В истории лежат только настоящие цены (epb_ingest читает снимки скраперов
     до fill_prices.py), так что наши собственные оценки сюда не попадают.

     hours — окно в часах; 0 или пропуск = всё, что записано. */
  var RATE_KEYS = null;
  function rateKeys() {
    if (RATE_KEYS) return RATE_KEYS;
    var vols = { '65k': 1, '130k': 131000 / 65000 };
    var terms = { '15m': 0.25, '1h': 1, '1d': 24, '3d': 72, '10d': 240 };
    RATE_KEYS = [];
    for (var v in vols) for (var t in terms) {
      RATE_KEYS.push([v + '_' + t + '_price', vols[v] * terms[t]]);
    }
    return RATE_KEYS;
  }

  EPB.rateSeries = function (hist, pid, hours) {
    var keys = rateKeys(), acc = {}, i, k;
    for (k = 0; k < keys.length; k++) {
      var one = EPB.series(hist, pid, keys[k][0]), div = keys[k][1];
      for (i = 0; i < one.length; i++) {
        var ts = one[i][0], r = one[i][1] / div;
        if (!isNum(r) || r <= 0) continue;
        if (!acc[ts]) acc[ts] = [0, 0];
        acc[ts][0] += r; acc[ts][1] += 1;
      }
    }
    var out = [];
    for (var key in acc) out.push([Number(key), acc[key][0] / acc[key][1]]);
    out.sort(function (a, b) { return a[0] - b[0]; });
    if (hours && out.length) {
      var from = out[out.length - 1][0] - hours * 3600000;
      out = out.filter(function (e) { return e[0] >= from; });
    }
    return out;
  };
  ...
  /* Inline SVG sparkline. Returns '' for fewer than 3 points — callers just
     ...
     A round cap on the last point says which end is now. The dashed
     start-level guide was removed on request: on a 24-hour window it marked
     yesterday's price, which is what the delta beside the chart already
     says in words. */
  ...
  EPB.initTracking = function () {
    ...
    function onClick(e) {
      ...
      /* Anchors, and nothing else. A second branch used to sit here for a
         whole-card click: it read data-go off the card and reported
         how='card'. index.html stopped opening cards with window.open a
         while ago - it stretches the card's own outbound link across the
         card instead, so that click arrives as an ordinary anchor and the
         keyboard can reach it too. data-go was never written into a single
         page after that change, so the branch could not fire and how='card'
         has never appeared in a report. Every exit is how='link' now. If a
         card is ever made clickable without a link again, add the attribute
         to the markup first - the handler on its own counts nothing. */
  ...
  /* The stylesheet has been asking for `epb-scrolled` for a while and nothing
     has been setting it: whatever used to do so is gone, and the live page
     carries only `epb-js`. That was harmless while the header always painted
     its own ground, and became a defect the moment that ground was made
     conditional on the class - the condition could never come true.

     It is what lets the bar carry no ground at the very top, so the glow
     crosses it rather than stopping at its edge, and take one back the instant
     there is content underneath that has to stay readable.

     `epb-banded` is deliberately NOT set here. Three rules wait on it too, but
     one of them pairs with a --band-h that some vanished code used to measure;
     turning it on without that is a change nobody asked for.

     rAF-throttled, and it runs once at boot so a page restored mid-scroll is
     right before it paints. */
  EPB.initHeaderState = function () {
    ...
    function apply() {
      ...
      root.classList.toggle('epb-scrolled', y > 4);
    }
    function onScroll() {
      if (ticking) return;
      ticking = true;
